...
### Template
*Template* is a file in BED format.
...
```-b|--bg-level <float>```<br>
In *test* mode the level of background, in percent of foreground. The background is a set of reads cut from fragments that are not intersected with the template binding sites. Fragments intersected with binding sites form the foreground.<br>
