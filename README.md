...
Processing:
  ...
  --fg-level <float>    in test mode the number of selected fragments within the features,
                        in percent.
  ...
  --strand-admix <OFF|ON>       turn on/off opposite strand admixture at the bound
                        of binding site. For the test mode only [OFF]
...
The sites of interest are represented by the set of features stated in a BED file, called *template*. 
...
Inclusion of background for all chromosomes can be avoided by generating direct alignments in SAM or BED, 
because it excludes the use of an external aligner.<br>
Default: ```ON```
...
To see the detailed warning message about each skipped feature, set verbose level to ```DBG``` (see ```--verbose```).<br>
Default: 1
...
However, in practice, all the experimental sequences demonstrate at the boundaries of the binding sites the presence of small admixtures of reads with the opposite strand (see ![figure](https://github.com/fnaumenko/isChIP/tree/master/pict/mix-strand.png)). 
...
UThis mode is used for specific tasks.
...
In paired end mode two FQ files are generated, with suffixes ‘_1’ and ‘_2’. 
BED and SAM files contain the immediate (direct) alignment. 
...
The input parameters of simulation process (fragment size distribution,  total number of sequenced reads,  background levels, amplification coefficient, type of fragment library) are adjusted correspondently to those in real ChIP-Seq experiment.
...
as it is shown in the ![figure](https://github.com/fnaumenko/isChIP/tree/master/auxil/sizeSelFilter.png). 
