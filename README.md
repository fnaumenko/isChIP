# isChIP
<b>I</b>n <b>S</b>ilico <b>ChIP</b>-seq is a fast realistic ChIP sequencing simulator.
...
<b>isChIP</b> generates output in one of three modes:
<i>test</i> – simulation of site of interest sequencing, the output is test sequences/alignment;<br>
<i>control</i> – simulation of control production, the output is “input” sequences/alignment;<br>
<i>regular</i> – simple regular cutting of reference chromosome, an auxiliary mode for special use.<br>
Test and control modes are distinguished only by involvement or elimination of sites of interest in a process. The sites of interest are represented by the set of features, stated in BED file, called template.  Template is a single optional parameter.

### Template
<i>Template</i> is a file in BED format.
...
If 4th field (name) and 5th field (score) are in a line, <b>isChIP</b> uses feature’s score to restrict appropriate enriched density.
...
If ```name``` is a directory, first <b>isChIP</b> searches .fa files in it.
...
To generate output for a single chromosome, name can be a path and option –c should be set, or name should indicate an appropriate reference file.<br>
In test mode the target references are determined by template. See also ```--bg-all``` and ```–c|--chr ``` options.

<b>isChIP</b> omits  'random' contigs and haplotype sequences.
...
```-b|--bg-level <float>```<br>
In <i>test</i> mode the level of background, in percent of foreground. The background is a set of reads cut from fragments that are not intersected with the template binding sites. Fragments intersected with binding sites form the foreground.<br>
...
```-c|--chr <chars>```<br>
Generate output for stated chromosome only, for instance ```–c 10```, ```--chr X```. It takes the same effect as the referencing to the chromosome file instead of directory. It is a strong option, it abolishes the impact of option ```--bg-all``` and all another chromosomes from <i>template</i>.<br>
Default: all

```--flat-len <int>```<br>
In <i>test</i> mode the boundary flattening length is a distance from the boundary of binding site on which the probability of fragment associating is increased from zero to maximum. Such it simulates the smoothing of enriched regions.<br>
Default: 0

```--bg-all <OFF|ON>```<br>
In <i>test</i> mode turn on/off background generation for all chromosomes, irrespective of chromosomes included in template. As we discovered, mapping by any aligner one or several chromosomes to the whole reference genome leads to short local lacks of alignment, “gaps”. Generation background for all chromosomes eliminates this issue.
...
In <i>test</i> mode turn on/off opposite strand admixture at the bound of binding site.<br>
In accordance with the nature of the binding of proteins, at the boundaries of the binding site, there should be reads with the same strand only. However, in practice, all the experimental sequences demonstrate at the boundaries of the binding sites the presence of small admixtures of reads with the opposite strand. This option allows you to specify a "theoretical" or "practical" simulation.<br>
Default: ON

```--ts-uni```<br>
In <i>test</i> mode ignores features scores if they are established in template. All features will be simulated with maximum score.
...
```-R|--regular <int>```<br>
<i>Regular</i> mode: write each read on starting position increased by stated shift.<br>
...
BED and SAM files state the immediate (direct) alignment. Any formats can be set, but as minimum one.
Default: FQ
...
Default: <i>test</i> mode: <b>mTest.*</b>, <i>control</i> mode: <b>mInput.*</b>, <i>regular</i> mode: <b>mRegular.*</b>
...
For this reason in <b>isChIP</b> the size selection has a pseudo normal distribution view, as it is shown on the figure.
...
Size selection filter is managed by 3 options: ```--sz-sel-sigma``` and ```--frag-len``` response to standard deviation and mean in standard normal distribution, and   frag-dev is half-width on which the standard distribution is “expanded” (d on the figure).
