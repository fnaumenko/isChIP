...
void Average::operator+=(UINT val)
{
	if( _keep )
		if( _summator < (ULLONG_MAX-val) ) {
			...
			if( !_keep && Imitator::Verbose(V_DEBUG) )
				Err("block count", "AVERAGE").Throw();
		}
		else {
			_keep = false;					//  cut off treatment due to summator overflow
			if( Imitator::Verbose(V_DEBUG) )	
				Err("block summation", "AVERAGE").Throw();
		}
}
...
void ChromsThreads::Print()
{
	...
	for(; it!=_threads.end(); it++) {
		cout << "thr " << int(it->Numb)
			 //<< ":\tweight " << setprecision(3) << double(it->sumSize)/minSize << MSGSEP_TAB;
			 << ":\tweight " << (it->sumSize) << MSGSEP_TAB;
...
AvrFragLengths::AvrFragLengths	(const string path)
{
	...
	TabFile file(_fileName = oss.str(), 2, true, TxtFile::ALL);
	const char* currLine;
...
void Imitator::ChromCutter::OutputReadCnt(Imitator::eGround gr, const string title)
{
	static BYTE charsCnt[2] = {0, 0};
	// print count of reads
	cout << title << setw(Imitator::DigitsCnt[gr]) << setfill(BLANK)
...
{
	if( !Verbose(V_RT) )	return;
	Mutex::Lock(Mutex::OUTPUT);
	if( !RegularMode )
		OutputReadCnt(Imitator::FG, TestMode ? "fg: " : StrEmpty);
	if( TestMode )
		OutputReadCnt(Imitator::BG, "  bg: ");
	cout << "\tN: " << sPercent(nts.CountN(), nts.Length(), 2, 4, false);
	if( !LetN )
...
{
	_isTerminated = true;
	cerr << "thread " << int(_thread.Numb) << ": " << msg << endl;
}
	...
	bool fgInFeature)
{
	...
	float	rvsSample;	// the probability of generating reversing Read: from -1 to 1
						...
						*currPos += fracShift;				// in case of BG fracShift is always 0
						if( !indGr && Imitator::StrandAdmix ) {	// FG and admix opposite strand?
							...
							if(fracCentre < start)
								...
								// likelihood of negative strand is decreasing from 1 to 0.5
								// while moving fragment right
								rvsSample = float(start - *currPos)/fracLen;
							else if(fracCentre > end)
								// if fragment is on the right site of BS boundary,
								// likelihood of negative strand is decreasing from 0.5 to 0
								// while moving fragment right
								rvsSample = float(end - *currPos)/fracLen;
							// if fragment's centre is inside of BS,
							// likelihood of neg strand is 0.5 (defined in the begining of loop)

							reverse = _lnDist.RequestSample(rvsSample);
						}
						else
							// likelihood of neg strand is always 0.5
							reverse = _lnDist.Boolean();
...
//		if < 0 than increasing to the end of "left" fragment (intersected with feature with its right end);
//		if 0 than fifty-fifty;
//		if > 0 than increasing to the end of "right" fragment (intersected with feature with its left end);
...
void Imitator::OutputReadCnt(eGround gr, const string title)
{
	ULONG cnt = TotalWrReadsCnts[gr]<<OutFile::PairedEnd();
	cout << title << cnt;		// print count
...
void Imitator::Execute()
{
	...
	if( Verbose(V_RES) ) {
		cout << "Total writed reads: " << (_oFile.Count() + TotalSlaveWrReadsCnt);
		if( TestMode ) {
...
bool Imitator::CutGenome	()
{
	...
	SetSample();
	if( slaves.Length() && Verbose(V_DEBUG))	cThreads.Print();	//return true;
...
void Imitator::SetSample()
{
	...
	if( !avrgs.Get(Amplification::Coefficient, &sizeFactor, &commonAvrg, &savedAvrg) ) {
		Timer timer;
		chrlen pos = 0;
		if( Imitator::Verbose(V_RT) ) {
			cout << "Samples file for given distribution" << MsgFileAbsent;
			fflush(stdout);
			timer.Start();
		}
		...
		if( Verbose(V_DEBUG) ) {
			...
			cout << SignDbg << "Numbers::\tfrags: " << LognormDistribution::CallsCnt()
				 << "\treads: " << chrCutter._wrReadsCnt[0] << endl;
			cout << SignDbg << "coeff of ampl: " << Amplification::SimpleMean() << endl;
		}
	}
	...
	if( Verbose(V_DEBUG) ) {
		cout << SignDbg << "AVERAGE:  frag: " << commonAvrg << "\tsaved: " << savedAvrg << endl;
		cout << SignDbg << "sizeFactor: " << sizeFactor << endl;
	}
	...
	if( Verbose(V_DEBUG) )
		cout << SignDbg << "CONTROL TOTAL READS TO WRITE: " << totalCnt << EOL
