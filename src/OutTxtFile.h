...
static struct DistrParams
{
	...
	//	@lnM: expectation of frag normal distribution
	//	@lnS: standard deviation of frag normal distribution
	//	@ssM: expectation of size sel normal distribution
	//	@ssS: standard deviation of size sel normal distribution
	//	@ssActive: true if size selection mode is ON
	static void Init(float lnM, float lnS, float ssM, float ssS, bool ssActive) {
		lnMean = lnM;
		lnSigma = lnS;
		ssMean = ssActive ? 
			(ssM == vUNDEF ? exp(lnM - lnS*lnS) : ssM):	// mode by default or explicit ss mean
			0;
		ssSigma = ssS;
	}
...
#define FRAG_MAX	INT_MAX		// maximum fragment length
...
public:
	...
	static inline void Init(sMode smode, ULONG rLimit) {
		maxFragCnt = rLimit >> (mode = smode);	// reduce twice in case if PE
	}
...
public:
	...
	inline static BYTE MaxLength() { return MaxLen; }
	...
	// Adds integer value to the Read name
	void AddVal(ULLONG val) { _len = _chrLen + sprintf(_name + _chrLen, "%d", val); }

	// Adds positions to the PE Read name
	void AddTwoPos(chrlen pos1, chrlen pos2)
	{ _len = _chrLen + sprintf(_name + _chrLen, "%d%c%d", pos1, Read::NmPos2Delimiter, pos2); }
};
...
protected:
	...
	void LineAddReadNameBack(BYTE mate, bool addDelim = true);
...
// 'BedROutFile' implements methods for writing Bed file
...
public:
	...
	//	@mate: mate number for PE Reads, or 0 for SE Read
	void AddRead(const char* read, bool reverse, BYTE mate=0);
...
private:
	...
	void AddRead(BYTE mate, const char* fields7_9, int len7_9,
		const char* read, const char* flag, chrlen pos);
...
public:
	...
	inline void AddRead(const char* read, chrlen pos, bool reverse) {
		AddRead(0, Fields7_9.c_str(), Fields7_9.length(), read, FLAG[reverse], ++pos);
	}
...
class WigOutFiles
{
	...
	public:
		...
		inline Coverage* OpenCover(chrid cID) { return At(cID).CreateCover(); }
...
// 'OutFiles' wraps test and control output files
class OutFiles
{
public:
	// Output file formats
...
private:
	...
	public:
		...
		static void Init(float sErrProb) {
			StrandErrProb = sErrProb;
			// set pAddReadInfo method
			if(Read::IsNameEmpty())		pAddReadInfo = &OutFiles::OutFile::RNameEmpty;
			else if(Read::IsNameNumb())	pAddReadInfo = &OutFiles::OutFile::RNameAddNumb;
			else if(Seq::IsPE())		pAddReadInfo = &OutFiles::OutFile::RNameAddTwoPos;
			else						pAddReadInfo = &OutFiles::OutFile::RNameAddVal;
		}

		// Sets sequense mode.
		//	@trial: if true, then set empty mode, otherwise current working mode
		inline static void SetSeqMode(bool trial)	{ 
			pAddRead = trial ? &OutFiles::OutFile::NotAddRead :
				(Seq::IsPE() ? &OutFiles::OutFile::AddReadPE : &OutFiles::OutFile::AddReadSE);
		}
...
public:
	...
	static void Init(oFormat fFormat, BYTE mapQual, bool wigStrand, float strandErrProb, bool zipped);
