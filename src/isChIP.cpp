...
Options::Option Options::_Options [] = {
	{ 'a', "amplif",	0,	tINT,	oTREAT, 1, 1, 2000, NULL, "coefficient of amplification" },
	{ 'b', "bg-level",	0,	tFLOAT,	oTREAT, 1, 0, 100, NULL,
	"number of selected fragments outside the features,\nin percent of foreground. For test mode only" },
	{ HPH, "fg-level",	0,	tFLOAT,	oTREAT, 100, 0, 100, NULL,
	"in test mode the number of selected fragments within the features,\nin percent.\nIn control mode  the number of selected fragments, in percent" },
	{ 'n', "cells",		0,	tLONG,	oTREAT, 1, 1, 1e7, NULL, "number of cells" },
	...
	{ HPH, "frag-len",	0,	tINT,	oFRAG, 200, 50, 400, NULL, "average size of selected fragments"	},
	{ HPH, "frag-dev",	0,	tINT,	oFRAG, 20, 0, 200, NULL, "deviation of selected fragments" },
	{ HPH, "bg-all",	0,	tENUM,	oTREAT, TRUE, 0, 2, (char*)Options::Booleans,
	"turn on/off generation background for all chromosomes.\nFor test mode only" },
	{ HPH, "bind-len",	0,	tINT,	oTREAT, 1, 1, 100, NULL,
	"minimum binding length. For test mode only" },
	{ HPH, "flat-len",	0,	tINT,	oTREAT, 0, 0, 200, NULL,
	...
	{ 'r', "read-len",	0,	tINT,	oREAD, 50, 20, 200, NULL, "length of output read" },
...
//{ HPH, "stdout",	0, tBOOL,	FALSE, 0, 0, NULL, "Write output to the 'standard output' filehandle instead of 'standart error'" },
	{ HPH, "smode",		0,	tENUM,	oTREAT, OutFile::mSE, OutFile::mSE, OutFile::mEmpty, (char*)smodes,
	...
	{ HPH,"ts-uni",	0,	tENUM,	oTREAT, FALSE, vUNDEF, 2, NULL, "uniform template score. For test mode only" },
	{ 'p',"threads",0,	tINT,	oTREAT, 1, 1, 50, NULL, "number of threads"},
	{ HPH,"debug",	0,	tENUM,	oTREAT, FALSE, vUNDEF, 2, NULL,
	...
	{ 'z',"gzip",	0,	tENUM,	oOUTPUT, FALSE, vUNDEF, 2, NULL, "compress output files with gzip"},
	{ 't',"time",	0,	tENUM,	oOTHER, FALSE, vUNDEF, 2, NULL, "print run time" },
	{ 'V',"verbose",0,	tENUM,	oOTHER, V_RT, V_CRIT, V_DEBUG+1, (char*)verbs,
...
void PrintImitParams(const ChromFiles& cFiles, const char* templName, OutFile& oFile)
{
	...
	cout << SignPar << "Reference: genome: " << cFiles.Path() << ", " << Chrom::Title << "s: ";
	...
	cout << EOL;
	if(templName)
		cout << SignPar << "Template: " << templName << EOL;
	oFile.Print(SignPar);
	if( RegularMode ) {
		...
		PrintReadInfo();
		if( TestMode ) {
			cout << SignPar << "Background for all chromosomes: ";
			if(bool(SAMPLE_BG()))	cout << Options::GetBoolean(oBG_ALL) << EOL;
			else					cout << "negligible due to zero background sample\n";
			cout << SignPar << "Input sample: foreground = " << SAMPLE_FG()
				 << PERS << GroupParSep << "background = " << SAMPLE_BG() << PERS << EOL;
			cout << SignPar << "Binding length: " << Options::GetIVal(oBS_LEN) << EOL;
			cout << SignPar << "Boundary flattening length: " << Options::GetIVal(oFLAT_LEN) << EOL;
			cout << SignPar << "Strand admixture: " << Options::GetBoolean(oSTRAND_MIX) << EOL;
			cout << SignPar << "Uniform template score: " 
		...
		if(SZ_SEL())
			cout << "length = " << Options::GetIVal(oFRAG_LEN) 
				 << GroupParSep << "deviation = " << Options::GetIVal(oFRAG_DEV)
				 << GroupParSep << "SD of the normal distribution = "
